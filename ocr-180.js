...
const drawBoundingBoxesAndRotatedText = async (flippedImageBuffer, boxes, outputPath, imageName) => {
  ...
  toFile(path.join(outputPath, `180_${imageName}`));
};
...
const performOCR = async (imagePath, outputPath) => {
  try {
    ...
    const flippedImageBuffer = await sharp(preprocessedImageBuffer).rotate(180).toBuffer();

    // Adjust coordinates for the horizontal flip
    const flippedBoxes = boxes.map(box => rotateCoordinates180(box, imageWidth, imageHeight));
    ...
    await drawBoundingBoxesAndRotatedText(flippedImageBuffer, flippedBoxes, outputPath, path.basename(imagePath));
...
const directoryPath = './OCR Test/New/Selected';
const outputPath = './OCR Test/New 180';
