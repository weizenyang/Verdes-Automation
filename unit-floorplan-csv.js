...
// const outputDir = './Arthouse/floorplan/Unit matched types/NEW/S1 240806 Flipped';
...
// const testDirectoryPath = './Arthouse/floorplan/segmented - distinct/POLISHED/S1 240806/TEST';
...
fs.readFile(jsonFilePath, 'utf8', (err, data) => {
  ...
  console.log(affectedUnits)
});


function updateLogs(groupName, unitName) {
  if (affectedTypes.includes(groupName)) {
    if (Object.prototype.hasOwnProperty.call(affectedUnits, groupName)) {
      if (Array.isArray(affectedUnits[groupName])) {
        if (!affectedUnits[groupName].includes(unitName)) {
          affectedUnits[groupName].push(unitName);
          affectedUnits[groupName].sort((a, b) => {
      ...
      affectedUnits[groupName] = [unitName]; // Initialize with an array containing unitName
    }
  }
}