...
const inputDirectory = './Floorplan Images/to be scaled';
const outputDirectory = './Floorplan Images/with border';
const outputResolution = { width: 4320, height: 4320 }; // Desired output resolution
const borderSize = 250; // Size of the border
const borderColor = { r: 255, g: 255, b: 255, alpha: 1 }; // Border color
...
async function addBorderAndScale(inputPath, outputPath, resolution, border, color, quality) {
    try {
        ...
        const edge = await sharp(inputPath).extract({ left: 0, top: height - 300, width: width, height: 200 }).toBuffer();
        ...
        const extractedColor = { r: r, g: g, b: b, alpha: 1 }
