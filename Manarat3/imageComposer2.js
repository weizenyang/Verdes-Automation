...
async function layerImages() {
  try {
    ...
    for (const group of imageGroup) {
      ...
      const rotationData = [... new Set(unitData.filter((e) => e.type == `${baseType.propertyType}_${baseType.standardOrPremium}_${baseType.bedroomCount}${baseType.flipped ? "f" : ""}_${baseType.variant}_${baseType.schema}_${baseType.floor}`).map((e) => {return e.rotation}))]
